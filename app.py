...
for i in range(2000):  # simulate 2000 time steps
    ...
    st.session_state.df["Temp_Roll"] = st.session_state.df["Temperature"].rolling(
        st.session_state.window, min_periods=1
    ).mean()
    ...
    recent_anomalies = st.session_state.df[st.session_state.df["Anomaly"] == -1].tail(10)
    table_area.dataframe(recent_anomalies)
    ...
    metrics_area.markdown(
        f"**Total readings:** {total_points}  |  "
        f"**Anomalies detected:** {anomaly_points}  |  "
        f"**% Anomalies:** {anomaly_points / total_points * 100:.2f}%"
    )
